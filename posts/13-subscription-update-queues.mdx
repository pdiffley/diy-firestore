...
only the changes that have been made without the risk of data being lost. We will create a
queue (I use the term loosely) for every client. When a
client's subscription is affected by a write, the update will be written to the queue.
The client can then retrieve all of the changes for its subscriptions from the
update queue.
...
For each subscription affected by the update, we delete any existing document 
with the same id (an old update that has not been retrieved yet) from its 
update queue if there is one, then insert the new document. Now we just
...
modifications are made it ensuring that the update queue always represents the 
