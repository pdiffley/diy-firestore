...
Firestore that distinguishes it over other databases. For any query, we can
make to the database, we can create a listener that receive's
updates whenever the database changes in a way that would affect that query's
results.
...
with expensive requests, almost all of which would return no new information to the
client.
...
#### Identify subscriptions affected by a write
...
To actually subscribe to a query in the first place, we will create another table `client_subscriptions`,
...
In practice, we would have this subscription funciton also return the initial query results as indicated in the todo.
