...
how Firestore works at all, it could be helpful to read the [Firestore docs](https://firebase.google.com/docs/firestore)
...
**collection_parent_path** and encompasses everything in a document's unique
path prior to its collection. For example, the collection_parent_path for the
...
will be ordered based on their numberic value (e.g. `1 < 2.5 < 3 < 3.5`).

## Queries

Following [Firestore's terminology](https://firebase.google.com/docs/firestore/query-data/queries), 
...
`<=`, `>`, `>=`, `==`, and `!=` "out of the box", without requiring that users
specify the need for them in advance.
...
```
Here we are specifying that we want to run queries on the 
...
```
Here we are specifying that the group of composite fields for all
...
const unsubscribeHandler = onSnapshot(q, (querySnapshot) => {
  const messages = querySnapshot.map((doc) => doc.data().message);
...
await runTransaction(db, async (transaction) => {
  const counterDoc = await transaction.get("/counter_collection/counter");
  const currentCount = counterDoc.data().count;
  transaction.update("/counter_collection/counter", {
...
complete. All other transactions attempting to modify the document will have
to retry, and they will ultimately fail if the 5-retry limit is reached.
Personally, when I have used Firestore's server side transactions in the
...
for example by managing subscriptions and transactions. However, in this series, I will 
