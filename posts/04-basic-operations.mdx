...
Right now these functions just take a document and insert or delete it from the documents table.
...
so we don't need to perform that check in the `write_document` function. 

Our write functions are simple right now, but we will be expanding them over the course of this series so that
...
```
Most of the work in these functions is done by the sql queries 
which retrieve data from the documents table based on the specified identifiers.
